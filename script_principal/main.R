...
normality_test<-function(filename=fn){
  ...
  print("Pour les résultats de test de normalité veuillez voir le fichier Excel test_normalite.xlsx")
}
...
#--------------------------- test de stationarité d'une période -----------------------------------
...
#--------------------------- test d'existence de l'effet ARCH pour tout les données -----------------------------------
...
arch_effect_test<-function(filename=fn){
  ...
  print("Pour les résultats de test d'existence d'effet arch  veuillez voir le fichier Excel test_effet_arch_stationarite.xlsx")
}
      ...
      for (j in 2:ncol(data)){
        ...
        if(arch_effect_exist(periode)){
          ...
          if(is_stationary(periode)){
            ...
            predicted_series[,j-1]<-c(as.vector(periode),forecasted_value@forecast$seriesFor[1:nrow(forecasted_value@forecast$seriesFor)]) 
          }else{
            forecasted_value<-arch_garch(diff_periode,paste("diff(",colnames(data)[j],")"),nb_pred=nb_pred)
            predicted_series[2:nrow(predicted_series),j-1]<-c(as.vector(diff_periode),forecasted_value@forecast$seriesFor[1:nrow(forecasted_value@forecast$seriesFor)])         
          }
        }else{
          #l'effet arch n'existe pas on utilise le model sarima
          if(is_stationary(periode)){
            #données stationnaire on travail sur les données originales
            forecated_value<-sarimaFit(periode,colnames(data)[j],nb_pred=nb_pred);  
            predicted_series[,j-1]<-c(as.vector(periode),as.vector(forecated_value[[1]]$pred))
          }else{
            #données NON stationnaire on travail sur la dérivée
            forecated_value<-sarimaFit(diff_periode,paste("diff(",colnames(data)[j],")"),nb_pred=nb_pred);
            predicted_series[2:nrow(predicted_series),j-1]<-c(as.vector(diff_periode),as.vector(forecated_value[[1]]$pred))
          }
      ...
      for (j in 2:ncol(data)){
        ...
        if(arch_effect_exist(periode)){
          ...
          if(is_stationary(periode)){
            ...
            forecated_value<-as.vector(forecast(modelFit,nb_pred)$mean)
            predicted_series[,j-1]<-c(as.vector(periode),forecated_value)
          }else{
            #données NON stationnaire on travail sur la dérivée
            ...
            plot(forecast(modelFit,nb_pred));
            forecated_value<-as.vector(forecast(modelFit,nb_pred)$mean)
            predicted_series[2:nrow(predicted_series),j-1]<-c(as.vector(diff_periode),forecated_value)
          }
          print(colnames(data)[j])  
        }        
      }      
